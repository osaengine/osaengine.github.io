...
description: "Первый бенчмарк AI-трейдеров на реальных деньгах. Китайские модели обыграли GPT-5 и Gemini. Разбираем результаты, стратегии победителей и что это значит для алготрейдинга."
date: 2026-03-10
...
Шесть LLM (GPT-5, Claude Sonnet 4.5, Gemini 2.5 Pro, Qwen 3 MAX, DeepSeek V3.1, Grok 4) получили по $10,000 и две недели торговали криптовалютой на [Hyperliquid DEX](https://hyperliquid.xyz/).

Результаты шокировали: **китайские модели обыграли западные с разгромным счётом**. Qwen 3 MAX выиграл. GPT-5 и Gemini потеряли больше 60% капитала.
...
1. **GPT-5** (OpenAI) — флагман, самая раскрученная модель
2. **Claude Sonnet 4.5** (Anthropic) — известна аналитическими способностями
3. **Gemini 2.5 Pro** (Google DeepMind) — мультимодальная, интеграция с Google Cloud
4. **Grok 4** (xAI / Elon Musk) — обучена на данных X (Twitter)
...
| DeepSeek V3.1 | $12,891 | +28.9% | -15% | 67 | 1.5 |
| Claude Sonnet 4.5 | $11,204 | +12.0% | -18% | 89 | 0.9 |
| Grok 4 | $9,687 | -3.1% | -22% | 124 | 0.2 |
| GPT-5 | $3,845 | **-61.6%** | **-68%** | 203 | -1.2 |
| Gemini 2.5 Pro | $3,412 | **-65.9%** | **-71%** | 187 | -1.4 |
...
2. **GPT-5 и Gemini потеряли > 60% капитала** за две недели
3. **Количество сделок коррелирует с убытками** — больше сделок = больше потери
4. **Claude Sonnet 4.5 — единственная западная модель в плюсе** (+12%)
...
- **GPT-5 и Gemini уже падали:** $7,265 и $6,864 (-25%)
...
## Провал GPT-5 и Gemini: что пошло не так?

### **Проблема 1: Overconfidence**

[GPT-5 переоценивал свои прогнозы](https://www.bitget.com/news/detail/12560605032631).
...
- **Действие:** GPT-5 открывает лонг 15 ETH на $2,580 с плечом 5x
...
- GPT-5 был прав в направлении (reversal случился)
...
- GPT-5 (203 сделки): 203 * 0.05% = **10.15% комиссий**

GPT-5 отдал биржникам 10% капитала просто на комиссиях.

## Claude Sonnet 4.5: почему выжил среди западных моделей
...
### **Ключевые отличия от GPT-5 и Gemini:**

1. **Консервативность:** Плечо max 2x (vs 10x у GPT-5)
2. **Меньше сделок:** 89 сделок (vs 203 у GPT-5)
...
GPT-5 использовал 5-10x leverage. Результат: -61.6%.
...
- Open-source: [DeepSeek V3.1 open-source](https://github.com/deepseek-ai/DeepSeek-V3)
- Дешевле: API DeepSeek в 10x дешевле GPT-5
- Специализация: Обучены на данных из высоковолатильных рынков

**Для алготрейдеров:**

Если вы используете LLM API, попробуйте DeepSeek. Возможно, он подойдёт лучше, чем GPT-5, и обойдётся в 10 раз дешевле.
...
Организаторы дали китайским моделям более подробные промпты. Или наоборот, GPT-5 получил плохой промпт.
